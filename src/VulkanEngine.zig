...
fn initSwapchain(self: *Self) void {
    ...
    checkVk(c.vk.CreateImageView(self.device, &depth_image_view_ci, vk_alloc_cbs, &self.depth_image_view)) catch @panic("Failed to create depth image view");
...
fn initCommands(self: *Self) void {
    ...
    for (&self.frames) |*frame| {
        checkVk(c.vk.CreateCommandPool(self.device, &command_pool_ci, vk_alloc_cbs, &frame.command_pool)) catch log.err("Failed to create command pool", .{});
        self.deletion_queue.append(
            self.allocator,
        ...
        checkVk(c.vk.AllocateCommandBuffers(self.device, &command_buffer_ai, &frame.main_command_buffer)) catch @panic("Failed to allocate command buffer");
    ...
    checkVk(c.vk.CreateCommandPool(self.device, &upload_command_pool_ci, vk_alloc_cbs, &self.upload_context.command_pool)) catch @panic("Failed to create upload command pool");
    self.deletion_queue.append(
        self.allocator,
    ...
    checkVk(c.vk.AllocateCommandBuffers(self.device, &upload_command_buffer_ai, &self.upload_context.command_buffer)) catch @panic("Failed to allocate upload command buffer");
}
...
fn initDefaultRenderpass(self: *Self) void {
    ...
    checkVk(c.vk.CreateRenderPass(self.device, &render_pass_create_info, vk_alloc_cbs, &self.render_pass)) catch @panic("Failed to create render pass");
...
fn initFramebuffers(self: *Self) void {
    ...
    for (self.swapchain_image_views, self.framebuffers) |view, *framebuffer| {
        ...
        checkVk(c.vk.CreateFramebuffer(self.device, &framebuffer_ci, vk_alloc_cbs, framebuffer)) catch @panic("Failed to create framebuffer");
        self.deletion_queue.append(
            self.allocator,
...
fn initSyncStructures(self: *Self) void {
    ...
    for (&self.frames) |*frame| {
        checkVk(c.vk.CreateSemaphore(self.device, &semaphore_ci, vk_alloc_cbs, &frame.present_semaphore)) catch @panic("Failed to create present semaphore");
        self.deletion_queue.append(
            self.allocator,
        ...
        checkVk(c.vk.CreateFence(self.device, &fence_ci, vk_alloc_cbs, &frame.render_fence)) catch @panic("Failed to create render fence");
        self.deletion_queue.append(
            self.allocator,
    ...
    checkVk(c.vk.CreateFence(self.device, &upload_fence_ci, vk_alloc_cbs, &self.upload_context.upload_fence)) catch @panic("Failed to create upload fence");
...
const PipelineBuilder = struct {
    ...
    fn build(self: PipelineBuilder, device: c.vk.Device, render_pass: c.vk.RenderPass) c.vk.Pipeline {
        ...
        checkVk(c.vk.CreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipeline_ci, vk_alloc_cbs, &pipeline)) catch {
...
fn initPipelines(self: *Self) void {
    ...
    checkVk(c.vk.CreatePipelineLayout(self.device, &pipeline_layout_ci, vk_alloc_cbs, &triangle_pipeline_layout)) catch @panic("Failed to create pipeline layout");
    self.deletion_queue.append(
        self.allocator,
    ...
    checkVk(c.vk.CreatePipelineLayout(self.device, &mesh_pipeline_layout_ci, vk_alloc_cbs, &mesh_pipeline_layout)) catch @panic("Failed to create mesh pipeline layout");
    self.deletion_queue.append(
        self.allocator,
    ...
    checkVk(c.vk.CreatePipelineLayout(self.device, &textured_pipe_layout_ci, vk_alloc_cbs, &textured_pipe_layout)) catch @panic("Failed to create textured mesh pipeline layout");
    self.deletion_queue.append(
        self.allocator,
...
fn initDescriptors(self: *Self) void {
    ...
    checkVk(c.vk.CreateDescriptorPool(self.device, &pool_ci, vk_alloc_cbs, &self.descriptor_pool)) catch @panic("Failed to create descriptor pool");
    ...
    checkVk(c.vk.CreateDescriptorSetLayout(self.device, &global_set_ci, vk_alloc_cbs, &self.global_set_layout)) catch @panic("Failed to create global descriptor set layout");
    ...
    checkVk(c.vk.CreateDescriptorSetLayout(self.device, &object_set_ci, vk_alloc_cbs, &self.object_set_layout)) catch @panic("Failed to create object descriptor set layout");
    ...
    self.camera_and_scene_buffer = self.createBuffer(camera_and_scene_buffer_size, c.vk.BUFFER_USAGE_UNIFORM_BUFFER_BIT, c.vma.MEMORY_USAGE_CPU_TO_GPU);
    ...
    checkVk(c.vk.AllocateDescriptorSets(self.device, &global_set_alloc_info, &self.camera_and_scene_set)) catch @panic("Failed to allocate global descriptor set");
    ...
    checkVk(c.vk.CreateDescriptorSetLayout(self.device, &texture_set_ci, vk_alloc_cbs, &self.single_texture_set_layout)) catch @panic("Failed to create texture descriptor set layout");
    ...
    for (0..FRAME_OVERLAP) |i| {
        ...
        checkVk(c.vk.AllocateDescriptorSets(self.device, &object_set_alloc_info, &self.frames[i].object_descriptor_set)) catch @panic("Failed to allocate object descriptor set");
        ...
        self.frames[i].object_buffer = self.createBuffer(MAX_OBJECTS * @sizeOf(GPUObjectData), c.vk.BUFFER_USAGE_STORAGE_BUFFER_BIT, c.vma.MEMORY_USAGE_CPU_TO_GPU);
        self.buffer_deletion_queue.append(
            self.allocator,
...
fn initScene(self: *Self) void {
    ...
    checkVk(c.vk.AllocateDescriptorSets(self.device, &descriptor_set_alloc_info, &material.texture_set)) catch @panic("Failed to allocate descriptor set");
    ...
    checkVk(c.vk.CreateSampler(self.device, &sampler_ci, vk_alloc_cbs, &sampler)) catch @panic("Failed to create sampler");
    self.deletion_queue.append(
        self.allocator,
...
fn initImgui(self: *Self) void {
    const pool_sizes = [_]c.vk.DescriptorPoolSize{
        .{
            .type = c.vk.DESCRIPTOR_TYPE_SAMPLER,
        ...
        .{
            .type = c.vk.DESCRIPTOR_TYPE_SAMPLED_IMAGE,
            .descriptorCount = 1000,
        },
        .{
            .type = c.vk.DESCRIPTOR_TYPE_STORAGE_IMAGE,
            .descriptorCount = 1000,
        },
        .{
            .type = c.vk.DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER,
            .descriptorCount = 1000,
        },
        .{
            .type = c.vk.DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER,
            .descriptorCount = 1000,
        },
        .{
            .type = c.vk.DESCRIPTOR_TYPE_UNIFORM_BUFFER,
        ...
        .{
            .type = c.vk.DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC,
            .descriptorCount = 1000,
        },
        .{
            .type = c.vk.DESCRIPTOR_TYPE_INPUT_ATTACHMENT,
            .descriptorCount = 1000,
        },
    };
    ...
    checkVk(c.vk.CreateDescriptorPool(self.device, &pool_ci, vk_alloc_cbs, &imgui_pool)) catch @panic("Failed to create imgui descriptor pool");
...
pub fn cleanup(self: *Self) void {
    checkVk(c.vk.DeviceWaitIdle(self.device)) catch @panic("Failed to wait for device idle");
...
fn loadTextures(self: *Self) void {
    ...
    checkVk(c.vk.CreateImageView(self.device, &image_view_ci, vk_alloc_cbs, &lost_empire.image_view)) catch @panic("Failed to create image view");
    self.deletion_queue.append(
        self.allocator,
...
fn draw(self: *Self) void {
    ...
    checkVk(c.vk.ResetCommandBuffer(cmd, 0)) catch @panic("Failed to reset command buffer");
    ...
    checkVk(c.vk.BeginCommandBuffer(cmd, &cmd_begin_info)) catch @panic("Failed to begin command buffer");
...
fn drawObjects(self: *Self, cmd: c.vk.CommandBuffer, objects: []RenderObject) void {
    ...
    for (objects, 0..) |object, index| {
        if (index == 0 or object.material != objects[index - 1].material) {
            ...
            c.vk.CmdBindDescriptorSets(cmd, c.vk.PIPELINE_BIND_POINT_GRAPHICS, object.material.pipeline_layout, 0, 1, &self.camera_and_scene_set, @as(u32, @intCast(uniform_offsets.len)), &uniform_offsets[0]);

            c.vk.CmdBindDescriptorSets(cmd, c.vk.PIPELINE_BIND_POINT_GRAPHICS, object.material.pipeline_layout, 1, 1, &self.getCurrentFrame().object_descriptor_set, 0, null);
        }

        if (object.material.texture_set != VK_NULL_HANDLE) {
            c.vk.CmdBindDescriptorSets(cmd, c.vk.PIPELINE_BIND_POINT_GRAPHICS, object.material.pipeline_layout, 2, 1, &object.material.texture_set, 0, null);
        }
...
fn createMaterial(self: *Self, pipeline: c.vk.Pipeline, pipeline_layout: c.vk.PipelineLayout, name: []const u8) *Material {
...
pub fn createBuffer(self: *Self, alloc_size: usize, usage: c.vk.BufferUsageFlags, memory_usage: c.vma.MemoryUsage) AllocatedBuffer {
...
pub fn immediateSubmit(self: *Self, submit_ctx: anytype) void {
    ...
    checkVk(c.vk.QueueSubmit(self.graphics_queue, 1, &submit_info, self.upload_context.upload_fence)) catch @panic("Failed to submit to graphics queue");

    checkVk(c.vk.WaitForFences(self.device, 1, &self.upload_context.upload_fence, c.vk.TRUE, 1_000_000_000)) catch @panic("Failed to wait for upload fence");
    checkVk(c.vk.ResetFences(self.device, 1, &self.upload_context.upload_fence)) catch @panic("Failed to reset upload fence");

    checkVk(c.vk.ResetCommandPool(self.device, self.upload_context.command_pool, 0)) catch @panic("Failed to reset command pool");
}
