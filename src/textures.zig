...
pub fn loadImageFromFile(engine: *Engine, filepath: []const u8) !Engine.AllocatedImage {
    ...
    const staging_buffer = engine.createBuffer(image_size, c.vk.BUFFER_USAGE_TRANSFER_SRC_BIT, c.vma.MEMORY_USAGE_CPU_ONLY);
    ...
    try checkVk(c.vma.MapMemory(engine.vma_allocator, staging_buffer.allocation, &data));
    ...
    const img_info = std.mem.zeroInit(c.vk.ImageCreateInfo, .{ .sType = c.vk.STRUCTURE_TYPE_IMAGE_CREATE_INFO, .imageType = c.vk.IMAGE_TYPE_2D, .format = format, .extent = extent, .mipLevels = 1, .arrayLayers = 1, .samples = c.vk.SAMPLE_COUNT_1_BIT, .tiling = c.vk.IMAGE_TILING_OPTIMAL, .usage = c.vk.IMAGE_USAGE_TRANSFER_DST_BIT | c.vk.IMAGE_USAGE_SAMPLED_BIT });
    ...
    try checkVk(c.vma.CreateImage(engine.vma_allocator, &img_info, &alloc_ci, &image, &allocation, null));
    if (allocation == null) {
